...
GLib에는 단방향과 양방향 양쪽 연결 리스트를 구현하는 데이터 구조가 준비되어 있음.
...
XML은 eXtensible Markup Language 의 줄임말이다. 굳이 우리말로 옮기면 '확장 가능한 마크업 언어' 정도의 의미.
XML은 웹(Web)에서 이용되는 HTML(HyperText Markup Language)에 아주 가까움.
양쪽 모두 구조화된 텍스트 데이터를 작성할 때 이용됨.
...
서버는 다양한 서비스에 번호를 지정해 클라이언트의 요청을 받음. 같은 IP 주소의 서버라도 특정번호로 된 서비스를 제공하고 다른 번호로는 다른 서비스를 제공.
이 번호를 포트 번호라고 부름. 이런 방식으로 프로그램은 같은 서버가 제공하는 여러 개의 서비스를 구별.

실제로는 포트 번호는 노드 간에 수없이 주고받는 통신의 식별 번호에 지나지 않음. 서버 측에 포트 번호가 주어지듯이 클라이언트 측에도 통신을 주고받는 포트 번호가 주어짐. 단, 클라이언트 측 포트 번호는 어떤 수 이상의 큰 숫자 값 중 비어 있는 값이 적절하게 할당되는 구조.

- 웰노운 포트

Well Known port 는 사실상의 표준으로 여겨지는 포트 번호와 서비의 대응을 보여주는 파일의 일부. 이 파일은 /etc/services에 있음.

- echo 서버

네트워크상의 서비스에서 가장 단순한 것이 'echo 서버'일 것임. echo 서버는 클라이언트가 보낸 요청 문자열을 그대로 돌려주기만 하는 정말 '메아리'같은 서버.
telnet을 클라이언트로 해서 로컬 호스트의 echo 서버에 액세스해보자. echo 서버는 7번 포트 번호로 클라이언트의 요청을 받음. 하지만 우분투 데스크톱 버전의 기본 설정으론 echo 서버에 액세스 할 수 없다.

설치가 안되있을 경우 폴더가 없을 수 있음. /etc/xinetd.d/echo 의 7행 주석으로 처리.
```
& sudo apt-cache search xinetd
$ sudo apt-get install xinetd
```

파일 수정한 뒤에 xinetd를 재시작하면 echo 서버가 요청을 처리할 수 있게 됨.
```
...
Connection closed.
```
에스케이프 시퀀스인 ^] (Ctrl키 + ']') 를 입력하고 telnet 명령으로 접속을 닫음.

> 간단한 서버 프로그램

list0912.c 작성.

// 네트워크 하는 도중 바빠져서 여기까지 일단 공부 완료!!!
// 추후에 시간되면 책 다시 빌려서 시작!!!
// 우분투 환경에서 C언어로 배우는 리눅스 프로그래밍
